...
using FindSimilarServices.Fingerprinting.Audio.Models;
using FindSimilarServices.Fingerprinting.Audio.Services;
using FindSimilarServices.Fingerprinting.Audio.Strides;
using FindSimilarServices.Fingerprinting.FFT;
using FindSimilarServices.Fingerprinting.Wavelets;
using FindSimilarServices.Fingerprinting.WorkUnitBuilder;
using FindSimilarServices.Fingerprinting.Configuration;
...
namespace FindSimilarServices.Fingerprinting
{
    public class FingerprintService
    {
            ...
            double[][] logarithmizedSpectrum, IStride stride, int fingerprintLength, int overlap, int topWavelets)
        {
            ...
            Debug.WriteLine("Created {1} Fingerprints from Log Spectrum - Execution Time: {0} ms", t.Stop().TotalMilliseconds, fingerprints.Count);
            return fingerprints;
        }
    }
}