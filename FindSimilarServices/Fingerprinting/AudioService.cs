...
namespace FindSimilarServices.Fingerprinting.Audio.Services
{
    public class AudioService : IAudioService
    {
        private readonly LomontFFT lomontFFT;
        ...
        public double[][] CreateSpectrogram(string pathToFilename, IWindowFunction windowFunction, int sampleRate, int overlap, int wdftSize)
        {
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                lomontFFT.TableFFT(complexSignal, true);
            ...
            float[] samples, IWindowFunction windowFunction, AudioServiceConfiguration configuration)
        {
            ...
            Debug.WriteLine("Create Log Spectrogram - Execution Time: {0} ms", t.Stop().TotalMilliseconds);
            return frames;
        }
